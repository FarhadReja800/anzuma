  ...
  return (
    <div className="group relative flex flex-col justify-between">
      ...
      <div className={`relative w-full overflow-hidden rounded-2xl bg-linear-to-br ${product.imageBg} border border-zinc-100 dark:border-zinc-900 flex items-center justify-center p-8 transition duration-300 group-hover:shadow-md`}>
        ...
        {/* Badge Tag */}
        <span className="absolute left-3 top-3 z-20 rounded-full bg-white px-2.5 py-1 text-[9px] font-bold text-zinc-950 shadow-sm dark:bg-zinc-900 dark:text-zinc-50 dark:border dark:border-zinc-800 uppercase tracking-wider">
          {product.tag}
        </span>
        ...
        <button 
          ...
          className="absolute right-3 top-3 z-20 h-8 w-8 rounded-full bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-red-500 transition-colors shadow-sm cursor-pointer"
        >
          <Heart className="h-4 w-4" />
        </button>

        {/* Hover Add to Cart overlay */}
        <div className="absolute inset-x-0 bottom-0 z-20 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 bg-gradient-to-t from-black/20 to-transparent flex justify-center">
          <Button 
            onClick={(e) => {
              e.preventDefault()
              e.stopPropagation()
