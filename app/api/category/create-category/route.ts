 ...
 * Proxies request to the Express backend to create a category.
 */
export async function POST(request: Request) {
  try {
    ...
    if (roleLower !== "superadmin" && roleLower !== "super-admin" && roleLower !== "admin") {
      return NextResponse.json(
        { success: false, error: "Forbidden: Only Admin or Super Admin can create categories." },
        { status: 403 }
      )
    }
    ...
    console.error("Error proxying create-category to backend:", error)
